...
★ 전력사용량 추세를 세 번째 신호로 추가(2026-09-01, market_energy_matching.py로
  ...
  ★★ 2026-09-23: "반경 안 건물 유일" 조건이 너무 엄격했다는 게 확인된 김에,
  좌표(반경 150m)와 건물명(코어명 부분일치)을 AND로 겹치는 조건으로 완화했다
  (market_by_radius_and_name()) - 반경 안에 건물이 여럿 있어도 그중 이름이
  겹치는 게 유일하면 채택. 17곳이 새로 매칭됐으나 검증 과정에서 5곳을 뺐다:
  "가락시장역"은 원래 있던 "가락시장"과 동일 건물이라 중복(먼저 큐레이션된
  이름 유지), "영등포시장역 3번·4번"은 "영등포시장역 1번"과 동일 건물이라
  중복(하나만 유지), "신월중앙시장"은 이번엔 실제 지번(강서구 화곡동 929-1)까지
  확인해 "화곡중앙시장"과 같은 건물임을 재확인(건물명 자체도 그냥 "중앙시장"이고
  주소가 화곡동이라 화곡중앙시장 쪽이 맞다고 판단, 2026-09-07 판단 재확인).
  12곳 매칭 중 에너지 데이터 없는 2곳 제외하면 최종 8곳 추가. 34 -> 42/277곳.
...
MARKET_ENERGY_TREND = {
    ...
    "답십리 건축자재시장": 28.6,
    # 5차 매칭 (2026-09-23, 좌표 반경 150m + 건물명 부분일치 결합 매칭,
    # 동일 건물 중복 3곳 제외)
    "가락시장": 1.3,
    "강서농산물도매시장": -0.5,
    "남서울상가": -1.2,
    "마전교지하쇼핑센터(구 한일상가)": 8.1,
    "석관시장": 4.3,
    "영등포시장역 1번": -5.0,
    "영천시장입구": -1.4,
    "자동차부품상가": -3.5,
