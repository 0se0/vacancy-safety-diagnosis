...
| 노후 대형상가·전통시장 안전등급(A~D) | **277곳** 산출 (D46·C66·B109·A56, 지도 표시는 좌표 확보된 171곳) |
...
flowchart LR
    ...
    subgraph B["체인 B · 상권 등급"]
        B1["점포수 시계열<br/>순증감률·폐업률 (277곳)"] --> B2["+ 전력사용량 추세<br/>(33/277곳)"]
...
- 상권 A~D 등급 중 전력사용량 신호까지 반영된 곳은 277곳 중 33곳 — 나머지는
  주소명 중복 등으로 안전하게 매칭되지 않거나 매칭은 됐지만 에너지 데이터
  자체가 없어서 제외. 재확장 과정에서 서로 다른 두 상권명이 같은 건물에
...
- 277곳 목록은 상권분석서비스 상권명에 "시장"·"상가"가 들어간 것을 기계적으로
  ...
  섞여 있음 — 실제로 이런 곳 중 한 곳(자양종합시장)은 점포수 변화가 0%라는
  이유로 최저위험(A) 등급을 받았는데, 이건 "안정적"이 아니라 "이미 죽어서 변화가
  없는" 경우라 모델이 구분을 못 하는 사례로 남겨둔다
