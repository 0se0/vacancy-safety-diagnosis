...
| 노후 대형상가·전통시장 안전등급(A~D) | **277곳** 산출 (D63·C61·B103·A50, 지도 표시는 좌표 확보된 171곳) |
...
flowchart LR
    ...
    subgraph B["체인 B · 상권 등급"]
        B1["점포수 시계열<br/>순증감률·폐업률 (277곳)"] --> B2["+ 전력사용량 추세<br/>(34/277곳)"]
...
- 상권 A~D 등급 중 전력사용량 신호까지 반영된 곳은 277곳 중 34곳 — 나머지는
  주소명 중복 등으로 안전하게 매칭되지 않거나 매칭은 됐지만 에너지 데이터
  자체가 없어서 제외. 건물명 대신 좌표 반경(150m) 매칭도 시도했지만 138곳
  중 1곳만 늘었다 — 반경 안에 건물이 유일해야 채택하는 안전장치가, "여러
  건물이 몰려 있어야 시장"이라는 정의 자체와 충돌해서 대부분 실패했다.
  재확장 과정에서 서로 다른 두 상권명이 같은 건물에
  매칭되는 경우도 2건 발견해(예: "화곡중앙시장"과 "신월중앙시장"이 같은
  건물로 잡힘) 원래 수작업 큐레이션 쪽을 남기고 기계적으로 확장된 쪽을
  제외함 — 상권 하나당 건물 여럿을 막는 기존 안전장치는 이 방향의 충돌은
  못 잡는다는 걸 확인한 사례
