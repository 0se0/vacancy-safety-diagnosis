...
| 실측 검증(서울 8개구, 303개 건물, 공공 API 실시간 조회) | 집합건축물 **92.2%**가 등록↔실제 불일치 |
...
전수 안전점검은 불가능하다. 인력은 한정돼 있고, 서울 8개구 상업용 건물만 해도
5만 건에 육박한다. 그렇다고 신고가 들어올 때까지 기다리면 늦다 — 세운상가처럼.
...
flowchart LR
    subgraph A["체인 A · 실측 검증"]
        ...
        A2 --> A3["8개구 303건 실측<br/>92.2% 불일치 확인"]
    end
